...
export function buildGetRecordsXML(params: {
  ...
  const { startPosition, maxRecords, queryText, productType, productId } =
    params;
...
export function parseCSWResponse(xmlText: string): CSWQueryResult {
  ...
  const searchResults =
    xmlDoc.getElementsByTagNameNS(
      ...
      "SearchResults",
    )[0] || xmlDoc.querySelector("SearchResults");

  const totalMatched = searchResults
    ? parseInt(searchResults.getAttribute("numberOfRecordsMatched") || "0", 10)
    : 0;
  const returned = searchResults
    ? parseInt(searchResults.getAttribute("numberOfRecordsReturned") || "0", 10)
    : 0;
  ...
  for (let i = 0; i < recordNodes.length; i++) {
    ...
    if (!bbox) {
      const lowerCorner = node
        .querySelector("LowerCorner")
        ?.textContent?.trim()
        ?.split(" ")
        .map(Number);
      const upperCorner = node
...
export async function fetchRasterRecords(
  params: FetchRasterRecordsParams,
): Promise<CSWQueryResult> {
  const {
    cswUrl,
    token,
    startPosition = 1,
  ...
  if (!response.ok) {
    throw new Error(
      `CSW Catalog query failed with HTTP status ${response.status}: ${response.statusText}`,
    );
  }
