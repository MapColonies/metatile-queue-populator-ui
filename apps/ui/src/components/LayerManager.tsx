      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Badge badgeContent={activeLayers.length} color="primary">
            <LayersIcon fontSize="small" color={activeLayers.length > 0 ? 'primary' : 'action'} />
          </Badge>
        ...
        <Box sx={{ p: 1.5, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          ...
          <Box>
            ...
            {activeLayers.length === 0 ? (
              ...
              <List dense sx={{ p: 0, display: 'flex', flexDirection: 'column', gap: 0.8 }}>
                {activeLayers.map((layer, index) => (
                  <Paper
                    ...
                    sx={{
                      ...
                      bgcolor: 'background.paper',
                      borderRadius: 1,
                      borderColor: layer.visible ? 'divider' : 'rgba(255, 255, 255, 0.05)',
                      opacity: layer.visible ? 1 : 0.6,
                    }}
                  >
                    {/* Layer Header */}
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 0.5 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, overflow: 'hidden', mr: 1 }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 0.5 }}>
                      <Typography variant="caption" sx={{ fontSize: '0.7rem', color: 'text.secondary', minWidth: 42 }}>
                        Opacity:
                      </Typography>
                      <Slider
                        size="small"
                        ...
                        min={0}
                        max={100}
                        onChange={(_, val) => onUpdateLayerOpacity(layer.id, (val as number) / 100)}
                        sx={{ py: 0.5 }}
                      />
                      <Typography variant="caption" sx={{ fontSize: '0.7rem', fontFamily: 'monospace', minWidth: 32, textAlign: 'right' }}>
