...
export const TileListForm: React.FC = () => {
  ...
  const handleTileChange = (id: string, field: keyof Omit<TileItem, 'id'>, value: string) => {
    const numValue = parseInt(value, 10);
    setTiles((prev) =>
      prev.map((tile) =>
        tile.id === id ? { ...tile, [field]: isNaN(numValue) ? 0 : numValue } : tile
      )
    );
  };
  ...
  const handleParsePaste = () => {
    try {
      ...
      if (trimmed.startsWith('[')) {
        // JSON format
        const parsedJson = JSON.parse(trimmed) as Array<{ z: number; x: number; y: number; metatile?: number }>;
      ...
      setToast({
        open: true,
        message: err.message || 'Failed to parse paste data. Expected JSON or CSV (z,x,y,metatile).',
        severity: 'error',
      });
    }
  };
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1.5 }}>
        ...
        <Tooltip title="Bulk Paste CSV or JSON">
          <IconButton size="small" color="primary" onClick={() => setPasteDialogOpen(true)}>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Provide precise zoom, x, y coordinates and metatile sizes to populate the queue.
      </Typography>
      ...
      <form onSubmit={handleSubmit}>
        <Stack spacing={2}>
          <TableContainer sx={{ maxHeight: 280, bgcolor: 'rgba(0,0,0,0.2)', borderRadius: 1 }}>
            <Table size="small" stickyHeader>
              <TableHead>
                <TableRow>
                  <TableCell sx={{ width: 65, bgcolor: 'background.paper' }}>Z</TableCell>
                  <TableCell sx={{ width: 85, bgcolor: 'background.paper' }}>X</TableCell>
                  <TableCell sx={{ width: 85, bgcolor: 'background.paper' }}>Y</TableCell>
                  <TableCell sx={{ width: 75, bgcolor: 'background.paper' }}>Meta</TableCell>
                  <TableCell sx={{ width: 40, bgcolor: 'background.paper', p: 0 }}></TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {tiles.map((tile) => (
                  <TableRow key={tile.id}>
                    <TableCell sx={{ p: 0.5 }}>
                      <TextField
                        ...
                        type="number"
                        value={tile.z}
                        onChange={(e) => handleTileChange(tile.id, 'z', e.target.value)}
                    ...
                    <TableCell sx={{ p: 0.5 }}>
                      <TextField
                        ...
                        type="number"
                        value={tile.x}
                        onChange={(e) => handleTileChange(tile.id, 'x', e.target.value)}
                      />
                    </TableCell>
                    <TableCell sx={{ p: 0.5 }}>
                      <TextField
                        ...
                        type="number"
                        value={tile.y}
                        onChange={(e) => handleTileChange(tile.id, 'y', e.target.value)}
                      />
                    </TableCell>
                    <TableCell sx={{ p: 0.5 }}>
                      <TextField
                        ...
                        type="number"
                        value={tile.metatile}
                        onChange={(e) => handleTileChange(tile.id, 'metatile', e.target.value)}
                        inputProps={{ min: 1 }}
                      />
                    </TableCell>
                    <TableCell sx={{ p: 0.5 }}>
                      <IconButton size="small" color="error" onClick={() => handleRemoveRow(tile.id)}>
      ...
      <Dialog open={pasteDialogOpen} onClose={() => setPasteDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>
            Paste CSV format (<code>z,x,y,metatile</code> or <code>z/x/y</code>) or JSON array:
          </Typography>
          <TextField
            multiline
            rows={8}
            fullWidth
            placeholder={"10,500,300,1\n10,501,300,1\n10/502/300\n\nOr JSON:\n[{\"z\":10,\"x\":500,\"y\":300,\"metatile\":1}]"}
            value={pasteText}
