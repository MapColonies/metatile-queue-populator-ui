...
export const TileEstimationWidget: React.FC<TileEstimationWidgetProps> = ({ selectedArea, zoomRange }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const response = await axios.post<EstimationResult>('/api/tiles/estimate', payload);
      ...
      {error ? (
        ...
        <Box sx={{ mt: 1 }}>
          ...
          {estimation.totalTiles > HIGH_TILE_THRESHOLD && (
            ...
            >
              Large generation volume (&gt;500k tiles). May take longer to complete.
            </Alert>
          )}
          ...
          <Collapse in={expanded}>
            <TableContainer sx={{ maxHeight: 150, mt: 0.5, bgcolor: 'background.paper', borderRadius: 1 }}>
              <Table size="small" stickyHeader>
                <TableHead>
                  <TableRow>
                    <TableCell sx={{ fontSize: '0.7rem', py: 0.5 }}>Zoom</TableCell>
                    <TableCell align="right" sx={{ fontSize: '0.7rem', py: 0.5 }}>Metatiles</TableCell>
                    <TableCell align="right" sx={{ fontSize: '0.7rem', py: 0.5 }}>Standard Tiles</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {estimation.breakdown.map((row) => (
                    <TableRow key={row.zoom}>
                      <TableCell sx={{ fontSize: '0.7rem', py: 0.4, fontWeight: 600 }}>Z{row.zoom}</TableCell>
                      <TableCell align="right" sx={{ fontSize: '0.7rem', py: 0.4 }}>
                        {row.metatiles.toLocaleString()}
                      </TableCell>
                      <TableCell align="right" sx={{ fontSize: '0.7rem', py: 0.4, color: 'text.secondary' }}>
