...
const PRODUCT_TYPES = ['ALL', 'Orthophoto', 'RasterVector', 'Elevation', 'DTM', 'DSM', 'Vector'];
...
export const RasterCatalogDrawer: React.FC<RasterCatalogDrawerProps> = ({
  ...
  const handleToggleLayer = async (record: RasterRecord) => {
    console.log('[RasterCatalogDrawer] handleToggleLayer called for:', record.productName, record.id, 'is active:', isLayerActive(record.id));
    if (isLayerActive(record.id)) {
      ...
      try {
        ...
        console.log('[RasterCatalogDrawer] Successfully added layer:', record.productName);
      } catch (err: any) {
  ...
  return (
    ...
    >
      ...
      {/* Filter & Search Bar */}
      <Box sx={{ p: 2, borderBottom: '1px solid', borderColor: 'divider', bgcolor: 'rgba(26, 34, 40, 0.95)', display: 'flex', flexDirection: 'column', gap: 1.5 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="caption" color="text.secondary">
            Found <strong>{totalMatched}</strong> raster layer{totalMatched === 1 ? '' : 's'}
          </Typography>
          <Typography variant="caption" color="text.secondary">
          ...
          records.map((record) => {
            ...
            return (
              <Card
                ...
                sx={{
                  flexShrink: 0,
                  bgcolor: active ? 'rgba(0, 163, 224, 0.12)' : 'rgba(30, 39, 46, 0.85)',
                  borderColor: active ? 'primary.main' : 'rgba(255, 255, 255, 0.12)',
                  borderRadius: 2,
                  transition: 'all 0.2s',
                  '&:hover': {
                    borderColor: active ? 'primary.light' : 'rgba(255, 255, 255, 0.25)',
                    boxShadow: 3,
                  },
                }}
              >
                <CardContent sx={{ p: 2, pb: 1.5 }}>
                  ...
                  {record.description && record.description !== 'undefined undefined undefined undefined' && (
                    <Typography
                      variant="body2"
                ...
                <CardActions sx={{ px: 2, py: 1.2, display: 'flex', justifyContent: 'space-between', gap: 1, bgcolor: 'rgba(0,0,0,0.15)' }}>
                  <Button
                    size="medium"
                    variant={active ? 'outlined' : 'contained'}
                    color={active ? 'error' : 'primary'}
                    startIcon={isAdding ? <CircularProgress size={16} /> : active ? <DeleteIcon /> : <AddIcon />}
